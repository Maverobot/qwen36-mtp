...
# ---------- 6. launcher -----------------------------------------------------
LAUNCH="$PREFIX/run-qwen36.sh"
log "Writing launcher: $LAUNCH"
cat > "$LAUNCH" <<EOF
#!/usr/bin/env bash
# Launch Qwen3.6-27B-MTP on RTX 4090 (Ada).
# Drop --ctx-size for faster prefill / more VRAM headroom; raise toward 262144 to use the full window.
set -e
